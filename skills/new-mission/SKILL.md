...
- **從機器取，不要憑印象拼**。零依賴取法（macOS／Linux 通用，不需 python）：
  ...
  🚫 **別用 `date '+%A'` 拿星期**——它給的是 locale 決定的 `Tuesday`／`星期二`，兩者都不是規格要的「（二）」；`date +%Z` 也不能當時區，那是 `CST` 這類縮寫、不是 IANA 名稱。上面改用 `%u`（1–7）對照表，與 locale 無關。
- **時區那兩行為什麼長這樣**：`readlink` 要帶 `-f`——有些發行版的 `/etc/localtime` 是**多層**符號連結，只解一層會拿到中繼路徑，接著被當成時區印出來，而「有值」正好躲過 `${tz:-時區未知}` 的防呆，這是最難發現的一種錯：格式完全合格、內容是錯的。`case` 那行就是在擋這件事——切不出 IANA 名稱時寧可空著印「時區未知」，也不要印一個路徑字串。⚠️ 已知限制：`$TZ` 若被設成 `PST8PDT` 這類舊式 POSIX 字串，會照原樣印出（那確實是機器在用的時區，只是不是 IANA 寫法）；🚫 別為了讓它「看起來合格」而改印系統時區，那會印出一個跟實際時間對不上的假名稱。
